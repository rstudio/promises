...
Pro](https://docs.posit.co/shiny-server/) to launch new processes as
...
> read [Advanced
> R](http://adv-r.had.co.nz/Computing-on-the-language.md)) may be
...
An asynchronous function call (which uses [the mirai
...
function is what ultimately makes promise objects useful. It is used to
register success and failure handlers on a promise. Its signature looks
like:
...
In the example above, we used the anonymous function shorthand
(`\(x) { }`) to write an anonymous function. `function(x) { }` would
behave just the same.

To help reduce the amount of in/out reading, we can use the R pipe
operator (`|>`):
...
**always returns immediately**, without invoking the callback function.
The callback function will be invoked sometime in the future—it could be
...
function has an important function beyond registering callbacks. It also
returns a promise—not the promise it takes as an argument, but a new,
distinct promise. This new promise gets fulfilled after the input
promise has resolved and the callback registered by
[`then()`](https://rstudio.github.io/promises/dev/reference/then.md) has
run; the return value of the callback is used to fulfill the new
promise.
...
In this case, after `p` is fulfilled with a data frame, `p2` will be
fulfilled with the number of rows of that data frame.

Because
[`then()`](https://rstudio.github.io/promises/dev/reference/then.md)
uses promises for both input and output, you can chain multiple
[`then()`](https://rstudio.github.io/promises/dev/reference/then.md)
...
This is not correct, as the `print(nrow(df))` stage will not only print
the desired value, but pass the return value of `print(nrow(.))`, which
is just `invisible(nrow(df))`, to the next stage.

For synchronous code, the [`{magrittr}` package offered the `%T>%`
(pronounced “tee”)
operator](https://magrittr.tidyverse.org/reference/tee.html), which
operates like a regular `|>` except that, after executing its right-hand
side, it returns its left-hand side value.

Similarly, for asynchronous code, you can use the `then(tee = TRUE)`
...
preceding `print(nrow(df))` has changed from
[`then()`](https://rstudio.github.io/promises/dev/reference/then.md) to
`then(tee = TRUE)`.
...
empty `onRejected = \(err) { }`, as you might think. Even though this
function has no code in its body, it still returns normally, and thus
would cause any errors to be caught and swallowed. That’s not the
...
function that is just a shorthand for `then(onRejected)`. It saves a
...
`tee = TRUE` to your `catch` call. These two lines are equivalent to the
previous code chunk:
...
error). When programming with promises, you can use the
[`finally()`](https://rstudio.github.io/promises/dev/reference/then.md)
call to do the same. The
[`finally()`](https://rstudio.github.io/promises/dev/reference/then.md)
function is similar to
[`then()`](https://rstudio.github.io/promises/dev/reference/then.md) but
...
Our
[`finally()`](https://rstudio.github.io/promises/dev/reference/then.md)
code expression makes sure the temp file is deleted when the operation
is done, regardless of whether it succeeded or failed.
