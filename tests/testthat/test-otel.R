  ...
  describe("OpenTelemetry integration", {
    ...
    describe("promise domain integration", {
      it("integrates with existing promise domains", {
        ...
        p <- with_promise_domain(counting_domain, {
          with_hybrid_ospan("test_operation", tracer = get_tracer(), {
            promise_resolve(42) |>
              then(function(x) x * 2)
          })
        })
    ...
    describe("error handling", {
      it("handles promise rejections gracefully", {
        ...
        records <- otelsdk::with_otel_record({
          p <- with_hybrid_ospan("test_span3", tracer = get_tracer(), {
            promise_reject("test error") |>
              catch(function(reason) {
                with_hybrid_ospan("catch_span3", tracer = get_tracer(), {
                  42
                })
              ...
              then(function(result) {
                ...
                with_hybrid_ospan("then_span3", tracer = get_tracer(), {
                  42
                })
              })
          })
      ...
      it("propagates regular errors", {
        ...
        expect_error(
          with_hybrid_ospan("test_span4", tracer = get_tracer(), {
            stop("regular error")
          }),
          "regular error"
        )
    ...
    it("maintains span context across parallel promise chains", {
      ...
      recording <- otelsdk::with_otel_record({
        ...
        chain1 <- with_hybrid_ospan("chain_1", tracer = get_tracer(), {
        ...
        chain2 <- with_hybrid_ospan("chain_2", tracer = get_tracer(), {
  ...
  it("allows nested calls without duplicate domain setup", {
    ...
    records <- otelsdk::with_otel_record({
      result <- with_ospan_promise_domain({
        with_hybrid_ospan("outer_span", tracer = get_tracer(), {
          # This nested call should be idempotent
          with_ospan_promise_domain({
            with_hybrid_ospan("inner_span", tracer = get_tracer(), {
              promise_resolve(42) |>
                then(function(x) {
                  # Another nested call
                  with_ospan_promise_domain({
