...
#' @describeIn promises-deprecated Use `with_otel_span_promise_domain()` instead
#' @export
with_ospan_promise_domain <- function(expr) {
  ...
  with_otel_span_promise_domain(expr)
}

#' @describeIn promises-deprecated Use `local_otel_span_promise_domain()` instead
#' @export
local_ospan_promise_domain <- function(envir = parent.frame()) {
  ...
  local_otel_span_promise_domain(envir = envir)
}
