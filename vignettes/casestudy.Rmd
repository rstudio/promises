...
You can see the [code changes here](https://github.com/rstudio/cranwhales/compare/async...async2?diff=split). The code has not changed that much, but I do feel it has lost some elegance and maintainability, simply because the code for each of the affected outputs now has one foot in the the render function and one foot in the future. If your app's total audience is a team of a hundred analysts and execs, you might want to just forgo the extra performance and stick with the clearer, more maintainable code. But if you have serious scaling needs, this is probably a small price to pay.
