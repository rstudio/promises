...
Note, it is recommended to use an anonymous function (`\(x) { }`) when writing multi-line code or when the `.` context is ambiguous.

```r
# Clearer intent of value
promise |>
  then(\(x) {
    cat("The operation completed!")
    print(x)
...
# Confusing usage of `.`
promise_resolve(sample(1:10)) |>
  then(~ {
    . |>
      # What is the `.` here? Sample vector? An item of `.` (the sample vector)?
      purrr::detect(~ . > 5)
  }) |>
  then(print)

# Clearer usage of values
promise_resolve(sample(1:10)) |>
  then(print) |>
  then(\(x) {
    x |>
      # What is the `.` here?
      purrr::detect(\(val) val > 5)
  }) |>
  then(print)
```

TODO: re-write to `|>` w/ `then()`
#### Ex:
```r
promise_resolve(palmerpenguins::penguins) |>
