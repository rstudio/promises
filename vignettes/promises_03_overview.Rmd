...
We can take the syntactic sugar a step further by using the *promise pipe*, a promise-aware version of a pipe operator. The promise pipe looks like `%then%` and performs most of the same tricks as `|>`, but adds the functionality of `then`.
...
(Note that the `%then%` operator only supports the `onFulfilled` part of `then()`, so it’s not useful for handling errors; there’s a separate `%catch%` operator for that. We’ll cover this below in the section on [Error Handling](#error-handling).)
...
promise %then%
  ...
  group_by(state) %then%
  summarise(pop = sum(population)) %then%
  arrange(desc(pop))
```
...
Similarly, for asynchronous code, you can use the `%then_tee%` operator, which is like `%then%` except that after execution it resolves using its input promise. The only difference in the corrected code below is the operator immediately preceding `print(nrow(.))` has changed from `%then%` to `%then_tee%`.

```r
# Correct.
promise %then%
  filter(year == 2006) %then_tee%
  print(nrow(.)) %then%
  group_by(state) %then%
  summarise(pop = sum(population)) %then%
  arrange(desc(pop))
...
There’s an error handling pipe operator `%catch%`, that works similar to `%then%` but it binds to `then(onRejected)` instead of `then(onFulfilled)`:

```r
mirai(somepkg::operation()) %catch%
  warning()
```
...
That’s a fair amount of boilerplate. Instead, you can either add `tee = TRUE` to your `catch` call, or equivalently, use the `%catch_tee%` operator. These two lines are equivalent to each other, and to the previous code chunk:

```r
promise |> catch(print, tee = TRUE)

promise %catch_tee% print()
```

## Cleaning up with `finally`

In synchronous programming, you use either`tryCatch(expr, finally = ...)` or `on.exit(...)` to perform tasks (usually relating to freeing resources or reverting temporary changes) regardless of whether the main logic succeeds or fails (throws an error). When programming with promises, you can use the `%finally%` operator or the `finally` call to do the same. The `finally` function is similar to `then` but it only takes a single callback that executes on both success and failure, and its return value is ignored.
