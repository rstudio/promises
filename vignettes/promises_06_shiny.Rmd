...
function(input, output, session) {
  output$plot <- renderPlot({
    mirai({ expensive_operation() }) %then%
      head(input$n) %then%
      plot()
  })
}
...
The `promises` library is necessary for the `%then%` operator. You may also want to use promise utility functions like `promise_all` and `promise_race`.
...
```R
output$table <- renderTable({
  mirai(read.csv(url), url = url) %then%
    filter(date == input$date)
})
```
...
Note that in this version, we cannot call `data(df)` inside the mirai, as this would cause the update to happen in the wrong process. Instead, we use the `%then%` operator to perform the assignment back in the main process once the mirai resolves.
...
function(input, output, session) {
  ...
  filteredData <- reactive({
    data() %then% filter(date == input$date)
  })

  output$table <- renderTable({
    filteredData() %then% head(5)
  })
}
```
