...
When the stars align like this, converting this code to async is literally as easy as replacing each `|>` with `%then%`:
...
whale_downloads <- reactive({
  promise_all(data_df = data(), whales_df = whales()) %then% with({
    data_df |>
...
```r
> promise_all(x = mirai("Hello"), y = mirai("World")) %then%
+   with({ paste(x, y) }) %then%
+   print()
[1] "Hello World"
```
...
Fortunately, there's a pretty good escape hatch for `|>`, and `%then%` inherited it too. Instead of a pipeline stage being a simple function call, you can put a `{` and `}` delimited code block, and inside of that code block, you can access the "it" value using a period (`.`).
...
For untidy code with a single promise object, just remember: pair a single `%then%` with a code block and you should be able to do almost anything.
...
data <- eventReactive(input$date, {
  ...
  mirai(
    ...
    { p$set(message = "Parsing data...") } %then%
    { mirai(
        {
          read_csv(path, col_types = "Dti---c-ci", progress = FALSE)
        },
