---
title: "Asynchronous programming in R and Shiny"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteIndexEntry{Asynchronous programming in R and Shiny}
...
An asynchronous function call (using `future_async`) will look instead like:

    promise <- future_async(read.csv("data.csv"))
...
The `then` function is what ultimately makes promise objects useful. It is used to register success and failure handlers on a promise. Its signature looks like:
...
If you don’t have a named function that does what you want, though, you still have an alternative to using anonymous functions, which can be a little verbose: you can use formulas to save a few keystrokes. These use the [tidyeval](https://cran.r-project.org/web/packages/rlang/vignettes/tidy-evaluation.html) model; if you’re not familiar with tidyeval, for now just know that you can access the value (or error) using `.` as a variable name.
...
would be this, when the operation is performed asynchronously:
    ...
    promise <- future_async(operation()) %>%
