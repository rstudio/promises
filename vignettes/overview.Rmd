...
If you don’t have a named function that does what you want, though, you still have an alternative to using anonymous functions, which can be a little verbose: you can use formulas to save a few keystrokes. These use the [tidyeval](https://CRAN.R-project.org/package=rlang/vignettes/tidy-evaluation.html) model; if you’re not familiar with tidyeval, for now just know that you can access the value (or error) using `.` as a variable name.
