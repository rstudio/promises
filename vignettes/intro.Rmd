---
title: "An informal intro to async in R and Shiny"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteIndexEntry{An informal intro to async in R and Shiny}
  %\VignetteEncoding{UTF-8}
---

# An informal intro to async in R and Shiny
...
and it has the value you requested, a (hypothetical) `read.csv.async` function
...
example, in the case of our (again, hypothetical) `read.csv.async` function, the
...
tricks](https://cran.r-project.org/web/packages/magrittr/vignettes/magrittr.html)
...
calling an async function:
    ...
    df_promise <- read.csv.async("data.csv")
...
async-compatible version of Shiny (TODO: put version number), all of the
...
functions come from, read [TODO: link].
...
discussing. [TODO: link]

And finally, there are `promise_all` and `promise_race`, both used to combine
multiple promises into a single promise. Learn more about them here. [TODO:
link]
...
Sorry. I told you it was complicated. If you understand this guide, the one
about error handling, and the one about creating promises, you’ll be 95% of the
way there.
...
* read.csv.async is a terrible example, unless it could be implemented without
data serialization. Change the example to something else — maybe downloading
instead of reading/parsing.
* Talk about how promise-pipe stages can themselves return promises (the “real”
promise chaining).
