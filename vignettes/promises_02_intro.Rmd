...
A lot like that! You can’t use the `|>` operator itself as we need to wrap the promise
context in a function that may be called with the incoming promise resolves. So whereas
...
The `then()` call here is the secret sauce. It’s called the *promise pipe*;
...
```r
df_promise |> then(\(df) df |> filter(state == "NY"))
```
...
`then(tee = TRUE)`, which we call the *promise tee* method (it’s analogous to the
