...
promise-compatible version of it: `%then%`. So whereas you might do this to a
regular data frame:
...
```r
library(dplyr)
read.csv.async("https://rstudio.github.io/promises/data.csv") %then%
  filter(state == "NY") %then%
  View()
```

The `%then%` operator here is the secret sauce. It’s called the *promise pipe*;
...
In many ways `%then%` does work like a regular pipe: it rewrites each stage’s
...
The first and most important difference is that `%then%` *must* take a promise
as input; that is, the left-hand side of the operator must be an expression that
yields a promise. The `%then%` will do the work of “extracting” the result value
from the promise, and passing that (unwrapped) result to the function call on
the right-hand side.

This last fact—that `%then%` passes an unwrapped, plain old, not-a-promise value
...
**So the left-hand side of `%then%` needs to be one of these special promise
...
```r
df_promise %then% filter(state == "NY")
```
...
```r
df_filtered_promise <- df_promise %then% filter(state == "NY")
```
...
interact with them using `%then%` or other promise-aware operators and
functions.

**Wait, there are other promise-aware operators and functions?**

Yes. The `%then%` is the one you’ll most commonly use, but there is a variant
`%then_tee%`, which we call the *promise tee* operator (it’s analogous to the
magrittr `%T>%` operator). The `%then_tee%` operator mostly acts like `%then%`, but
instead of returning a promise for the result value, it returns the original
value instead. Meaning `p %then_tee% cat("\n")` won’t return a promise for the
...
There’s also `%catch%`, and its tee version, `%catch_tee%`, which are used for error
...
And finally, there are `promise_all` / `%all%`, `promise_race` / `%race%`, and `promise_map`, used to combine
