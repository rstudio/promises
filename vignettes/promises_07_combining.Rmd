...
The most common pattern for combining promises is gathering, where you have two or more promises in hand and you want to use all of their results in a computation. The `promise_all` function and `%all%` promise operator are designed for this. Their signatures looks like:
...
result <- promise_all(a = a, b = b) %then% {
  rbind(.$a, .$b)
}
```
...
Similar to `promise_all`'s `%all%`, `promise_race` has a `%race%` operator for convenience.
