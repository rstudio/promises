...
* Deprecated OpenTelemetry functions to use consistent naming and better reflect their behavior:
  * `with_ospan_async()` - Please use `with_hybrid_otel_span()` instead. This name change reflects handling of both synchronous and asynchronous operations, similar to `hybrid_then()`
