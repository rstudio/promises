...
* Renamed `with_ospan_async()` to `with_hybrid_ospan()` to better reflect its behavior of handling both synchronous and asynchronous operations, similar to `hybrid_then()`. Updated documentation and examples accordingly. `with_ospan_async()` is now deprecated. (#198)
